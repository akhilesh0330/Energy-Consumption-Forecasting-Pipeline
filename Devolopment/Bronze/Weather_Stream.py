...
energy_df = (
    spark.read
    .format("parquet")
    .load(base_transformed_path + "weather_stream_v2.parquet")
)
...
target_table = "bronze.bronze_sch.Weather_Stream"

try:

    (
        weather_bronze.write
        .format("delta")
        .mode("append")
...
FILE_NAME = "weather_stream_v2.parquet"
TARGET_TABLE = "bronze.bronze_sch.Weather_Stream"
...
try:
    ...
    inserted_rows = weather_bronze.count()
